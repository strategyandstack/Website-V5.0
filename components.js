    ...
    return `
        <div class="blueprint-module grid lg:grid-cols-2 gap-16 items-center">
            <div class="order-2 lg:order-1">
                ...
                <p class="text-gray-400 text-lg mb-8 leading-relaxed">${bp.description}</p>
                ${bp.timeline ? `<p class="text-sm text-white/50 mb-8"><strong class="text-white/70">Timeline:</strong> ${bp.timeline}</p>` : ''}
                <div class="grid grid-cols-2 gap-4 mb-8">
                    <div class="p-5 rounded-2xl bg-white/[0.03] border border-white/5">
                        <div class="text-[10px] text-gray-500 uppercase font-black mb-2 tracking-widest">Our Blueprint</div>
                        <div class="text-xl font-bold text-white">${bp.comparison.blueprint_hours} Hours</div>
                    </div>
                    <div class="p-5 rounded-2xl bg-white/[0.03] border border-white/5">
                        <div class="text-[10px] text-gray-500 uppercase font-black mb-2 tracking-widest">Manual Approach</div>
                        <div class="text-xl font-bold text-gray-500">${bp.comparison.manual_hours} Hours</div>
                    </div>
                </div>
                ...
                <div class="flex items-center gap-6">
                    ...
                    <a href="https://app.apollo.io/#/meet/managed-meetings/eddie/hor-57g-ivh/start" target="_blank" rel="noopener noreferrer" class="btn-secondary px-8 py-4 text-sm">
                        Book Strategy Call
                    </a>
                </div>
            </div>
...
export const createPricingCard = (pkg) => {
    ...
    return `
        <div class="glass-card p-10 rounded-[3rem] flex flex-col h-full ${isPopular ? 'border-white/30 scale-105 z-10' : 'border-white/10'}">
            ${isPopular ? '<div class="bg-white text-black text-[10px] font-black px-4 py-1.5 rounded-full uppercase self-start mb-8 tracking-widest">Most Popular</div>' : ''}
            <h3 class="text-2xl font-bold mb-2 tracking-tight">${pkg.name}</h3>
            <p class="text-xs text-white/40 font-black uppercase tracking-wider mb-4">${pkg.tagline}</p>
            <div class="mb-6">
                <span class="text-4xl font-black">${isCustom ? 'Custom' : pkg.price}</span>
            </div>
            <p class="text-gray-400 text-sm mb-6">${pkg.description}</p>
            ${pkg.savings ? `<p class="text-xs text-green-400 font-bold mb-6">${pkg.savings}</p>` : ''}
            ...
            <a href="https://app.apollo.io/#/meet/managed-meetings/eddie/hor-57g-ivh/start" target="_blank" rel="noopener noreferrer" class="w-full py-5 rounded-2xl ${isPopular ? 'bg-white text-black' : 'bg-white/5 border border-white/10 hover:bg-white/10'} font-black text-xs uppercase tracking-widest transition-all block text-center">
                Book Strategy Call
            </a>
        </div>
    `;
