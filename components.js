export const createValuePropCard = (title, description, icon) => {
    return `
        <div class="glass-card p-8 rounded-[2rem] border-white/10 group hover:border-white/30 transition-all duration-500">
            <div class="w-12 h-12 bg-white/5 rounded-2xl flex items-center justify-center mb-6 border border-white/10 group-hover:scale-110 transition-transform">
        ...
        <div class="blueprint-module grid lg:grid-cols-2 gap-12 items-center">
            <div class="order-2 lg:order-1">
                <div class="flex items-center gap-6 mb-6">
                    <span class="text-6xl font-black blueprint-number" style="color: rgba(255, 255, 255, 0.25);">${String(bp.number).padStart(2, '0')}</span>
                    <div>
            ...
            <div class="order-1 lg:order-2">
                <div class="glass-card p-10 rounded-[3rem] relative overflow-hidden group">
                    ...
                    <div class="relative z-10 flex flex-col items-center text-center">
                        ...
                        <h4 class="text-lg font-bold mb-4">Architecture Logic</h4>
...
export const createPricingCard = (pkg) => {
    ...
    return `
        <div class="glass-card p-8 rounded-[2.5rem] flex flex-col h-full ${isPopular ? 'border-white/30 scale-105 z-10' : 'border-white/10'}">
            ${isPopular ? '<div class="bg-white text-black text-[10px] font-black px-4 py-1.5 rounded-full uppercase self-start mb-6 tracking-widest">Most Popular</div>' : ''}
            <h3 class="text-xl font-bold mb-2 tracking-tight">${pkg.name}</h3>
            ...
            ${pkg.savings ? `<p class="text-xs text-green-400 font-bold mb-4">${pkg.savings}</p>` : ''}
            ...
            <a href="https://app.apollo.io/#/meet/managed-meetings/eddie/hor-57g-ivh/start" target="_blank" rel="noopener noreferrer" class="w-full py-4 rounded-2xl ${isPopular ? 'bg-white text-black' : 'bg-white/5 border border-white/10 hover:bg-white/10'} font-black text-xs uppercase tracking-widest transition-all block text-center">
                Book Strategy Call
            </a>
        </div>
    `;
